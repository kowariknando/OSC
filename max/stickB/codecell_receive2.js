...
//
//   Categorical channels (state, act, steps, bat, sleep, timeout) pass through
//   untouched - it makes no sense to smooth or auto-range an integer state code,
//   a step count, or the sleep/timeout state flags.
//
...
// Outlets 12/13 were added for sleep mode; they carry discrete state, so they
// are listed in PASSTHROUGH below and skip all smoothing / auto-range.
...
// ---- per-channel adaptive state (index 0..NUM_CHANNELS-1) ----
...
function resetRange() {
    for (var ch = 0; ch < NUM_CHANNELS; ch++) resetChannel(ch);
}
