# Single-variable multi-mapping plugins

Four independent Max for Live devices, each decoding **one** CodeCell channel and
routing it to **five independent mapping destinations**. One prop (e.g. a gyro
diabolo) can drive five different Ableton parameters at once, each with its own
smoothing so each reacts with a different amount of organic glide.

Each device shows **5 rows**, and each row has its own controls:

```
#   Smooth   Depth  Min  Max   MAP  Dest      Clear
1   (dial)    [ ]   [ ]  [ ]   (o)  target…   (x)
2   (dial)    [ ]   [ ]  [ ]   (o)  target…   (x)
... 5 rows
```

- **Smooth** — a dial (0–100, value shown below) controlling how organic *that
  row's* curve is: 0 = raw/snappy, 100 = very smooth. It's a mappable Ableton
  parameter (`Cmd+M` → move a MIDI knob), one per row.
- **Depth / Min / Max** — how far and within what range this row drives its target.
- **MAP / CLEAR** — pick / unpick which Ableton parameter this row controls
  (click MAP, then click any parameter in Live).

So: same sensor value → 5 rows → 5 Ableton parameters, each smoothed and scaled
on its own.
...
Each `.js` decodes UDP → shared auto-range → noise floor → **five per-lane
smoothers**. There is **no `setvar`**, so the old "the `.amxd` overrides the JS
suffix" trap cannot happen here.

Each device has **six outlets**:
- **outlets 0–4** — the sensor value smoothed five different ways, one per row.
- **outlet 5** — the battery, raw (0–100 %, `101` = charging, `102` = USB, no
  battery). Sent by the firmware every ~5 s regardless of `SENSOR_PROFILE`, so
  even a one-sensor prop reports it. Shown as `Battery: N%` plus a dot that pulses
  on each ping — your at-a-glance "the board is alive" indicator.

Auto-range is **shared** (it's a property of the sensor); only the **smoothing is
per-row**, so each Smooth dial changes just its own row.

## ⚠️ The `.amxd` files are auto-generated and UNVERIFIED

They were carved programmatically from `stickB`: five channel strips kept as the
five mapping rows, the js repointed to the multi-lane single-variable file (6
outlets), a `live.dial` **Smooth** control added per row, the battery readout
rewired to outlet 5, and the whole presentation re-laid-out into 5 spaced rows.
The structure was validated (no dangling wires, 6 js outlets, 5 dials + 5
`live.remote~`, parameter registry consistent, no duplicate parameter names),
**but they have not been opened in Max/Ableton.** Before trusting them:

1. Drop each on a track and confirm it loads with **no Max console errors**.
2. Confirm a row's number moves when the prop streams, and that MAP maps.
3. Confirm the **Smooth dials render as knobs** with a value below. If `live.dial`
   misbehaves, tell me — the safe fallback is the proven `live.numbox`.

Layout coordinates were set blind, so spacing may need a nudge in Max — that's
