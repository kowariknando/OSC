...
**Waking is by movement: pick the prop up and move it.** While asleep the board
is fully dark and draws almost nothing (~40 µA): it deep-sleeps on an RTC timer
and wakes itself every couple of seconds only long enough to re-power the IMU
and check for motion. A still prop drops straight back to sleep without ever
turning the Wi-Fi radio on, so Ableton never sees it; a prop that is being moved
boots fully, reconnects and streams again. Keep moving it for a second or two so
a check lands while it is moving.

Because the radio is off while asleep, **no network message can wake a board** —
that is a hardware fact, not a missing feature. Waking is a full reboot, so
expect Wi-Fi to reconnect in a few seconds and the IMU to need its usual 10–30 s
to settle.

We poll for motion rather than arm the BNO085's tap interrupt on purpose: that
interrupt shares the ESP32's deep-sleep wake pin and, with the full sensor set
streaming, spuriously re-woke the board on battery. The RTC-timer poll is immune
to that and keeps the board genuinely dark. Three constants at the top of
`main.cpp` tune it: `sleepPollSeconds` (how often it checks — lower = snappier
wake, higher = longer battery), `pollCheckMs` (how long each check samples), and
`wakeConfirmGyro` (how much movement counts as "wake me").
